...
@section('content')
    ...
    <div class="max-w-xl mx-auto p-4">
        <form action="/inventori/{{ $inventory->id }}" method="POST" class="bg-white shadow-md rounded px-6 py-8 space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="nama" class="font-medium">Nama Alat</label>
                <input type="text" name="nama" value="{{ old('nama', $inventory->nama) }}"
            ...
            <div>
                ...
                <select name="jenis_id" id="jenis_id"
                    ...
                    @foreach ($jenisAlats as $jenis)
                        <option value="{{ $jenis->id }}" {{ $inventory->jenis_id == $jenis->id ? 'selected' : '' }}>
                            {{ $jenis->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="new_jenis" id="new_jenis"
                    class="w-full border rounded px-3 py-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            ...
            <div>
                ...
                <input type="text" name="harga" id="harga" inputmode="numeric"
                    class="block w-full px-4 py-2 text-sm border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition {{ $errors->has('harga') ? 'border-red-500' : 'border-gray-300' }}"
                    value="{{ old('harga', 'Rp ' . number_format($inventory->harga, 0, ',', '.')) }}"
                    placeholder="Masukkan harga (contoh: Rp 2.500.000)">
                @error('harga')
            ...
            <div>
                ...
                <input type="number" name="pemakaian" value="{{ old('pemakaian', $inventory->pemakaian) }}"
                    class="w-full border rounded px-3 py-2 focus:ring-blue-500 focus:border-blue-500" required>
            </div>

            <div>
                <label for="status" class="font-medium">Status</label>
                <select name="status" class="w-full border rounded px-3 py-2 focus:ring-blue-500 focus:border-blue-500" required>
                    <option value="tersedia" {{ $inventory->status == 'tersedia' ? 'selected' : '' }}>Tersedia</option>
                    <option value="disewa" {{ $inventory->status == 'disewa' ? 'selected' : '' }}>Disewa</option>
                    <option value="perawatan" {{ $inventory->status == 'perawatan' ? 'selected' : '' }}>Perawatan</option>
                </select>
            </div>
